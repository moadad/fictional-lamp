(function(){
  ...
  const Api={
    ...
    login,
    summary:(force=false)=>{if(force)clear('summary');return request('summary',attachAuth(force?{force:'true'}:{}),{ttl:force?0:60000,dedupe:!force})},
    dashboard:(ready=false,force=false)=>{if(force)clear('getDashboardClients');return request('getDashboardClients',attachAuth({ready:ready?'true':'false',...(force?{force:'true'}:{})}),{ttl:force?0:45000,dedupe:!force})},
    clientModels:(client,ready=false,force=false)=>{if(force)clear('getClientModels');return request('getClientModels',attachAuth({client,ready:ready?'true':'false',...(force?{force:'true'}:{})}),{ttl:force?0:45000,dedupe:!force})},
    models:async(force=false)=>{if(force)clear('getModelsByPrefix');const params=attachAuth(force?{force:'true'}:{}),actions=['getModelsByPrefix','modelsByPrefix','models'];let last=null;for(const action of actions){const res=await request(action,params,{ttl:force?0:60000,dedupe:!force});last=res;if(!(res&&res.ok===false&&/إجراء غير معروف|unknown action/i.test(String(res.error||''))))return res}return last},
    prices:async()=>{const params=attachAuth();let res=await request('getModelPrices',params,{ttl:60000,dedupe:true});if(res&&res.ok===false&&/إجراء غير معروف|unknown action/i.test(String(res.error||'')))res=await request('prices',params,{ttl:60000,dedupe:true});return res},
